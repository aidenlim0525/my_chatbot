    ...
    if prompt.strip() == "상담 종료":
        ...
        if answered == 0:
            ...
            if len(predicted_scores) >= 9 and predicted_scores[8] >= 1:
                st.error("⚠️ 자살 관련 문항에 응답이 있거나 예측되었습니다. 전문가 상담을 권장합니다.")

            if user_name:
                sheet.append_row([user_name, total, level, f"{answered}/9 응답", "예측 점수 사용"])
                st.balloons()
                st.info("예측 결과를 포함해 Google 시트에 저장했습니다.")
    else:
        # 정상 상담 흐름
